---
title: "투자일지"
description: "주간 포트폴리오 변화와 매매 근거를 기록하는 투자일지 카테고리입니다."
layout: default
category: investment-journal
category_board: true
hide_topbar: true
seo:
  type: webpage
...
{% assign board_posts = site.posts | where_exp: "post", "post.categories contains 'investment-journal'" | sort: 'date' | reverse %}
...
<section class="category-board" data-category-board data-per-page="8">
  ...
  <nav class="filter-row" aria-label="카테고리 필터">
    ...
    <a class="filter-tab active" href="{{ '/categories/investment-journal/' | relative_url }}">투자일지</a>
      ...
      {% if board_posts.size > 0 %}
        {% for post in board_posts %}
          <a class="post-item" href="{{ post.url | relative_url }}">
            <span class="post-item__cat">투자일지</span>
