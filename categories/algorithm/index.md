---
title: "Algorithm"
description: "문제 해결 전략과 자료구조 및 알고리즘 학습 내용을 정리합니다."
layout: default
category: algorithm
category_board: true
hide_topbar: true
...
{% assign board_posts = site.posts | where_exp: "post", "post.categories contains 'algorithm'" | sort: 'date' | reverse %}
...
<section class="category-board" data-category-board data-per-page="8">
  ...
  <nav class="filter-row" aria-label="카테고리 필터">
    ...
    <a class="filter-tab active" href="{{ '/categories/algorithm/' | relative_url }}">Algorithm</a>
      ...
      {% if board_posts.size > 0 %}
        {% for post in board_posts %}
          <a class="post-item" href="{{ post.url | relative_url }}">
            <span class="post-item__cat">Algorithm</span>
